var ann = function(options) {
	...
	function initialise() {
		...
		function startWeightsGaussian(n) {
			function gaussian(n, m) {
				var mean = 0,
					sd = (1/Math.sqrt(n));
				return (1/(sd * Math.sqrt(2 * Math.PI)))(Math.pow(Math.E, -((Math.pow((m - mean), 2))/2(Math.pow(sd, 2))));
			}
	...
	function train(trainingSet, validationSet) {
		...
		function backwardPass(exampleOutput) {
			...
			for(var i = (layers.length - 1); i > 0; i--) {
				for(var j = (layers[i].length - 1); j >= 0; j--) {
					...
					pID = p.getID();
					deltas[pID] = ((perceptron.getType() === 'output') ? (exampleOutput - activations[pID]) || (sumOfOutputWeightings(perceptron)));
		...
		var count = 0, continueLoop = true, outputs = [], example;
