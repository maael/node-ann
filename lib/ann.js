var ann = function(options) {
    ...
    */
    ...
    function forwardPass(example, activations) {
        ...
        for(var i = 0; i < inputs.length; i++) {
            activations[inputs[i].getID()] = ((example.hasOwnProperty('inputs')) ? example.inputs[i] : example);
        }
    ...
    */
    function solve(solveSet) {
        ...
        activations = forwardPass(solveSet, activations);
        return activations[outputPerceptrons[(outputPerceptrons.length - 1)].getID()];
    }
    /*
